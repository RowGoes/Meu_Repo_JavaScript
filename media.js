...
function calcularMedia() {
  ...
  if (media === 0)  {
    mensagem = "Infelizmente você zerou a prova :(. Tente novamente numa próxima!";
    ...
    mensagem = `Parabéns! Você está na média com ${media.toFixed(1)}!`;
  } if (media >= 7.1 && media <= 9.9) {
    mensagem = `Você arrasou! Sua média é ${media.toFixed(1)}!`;
  } if (media === 10) {
    mensagem = `Você é barril dobrado! Brocou! Sua média é ${media.toFixed(1)}!`;
  }
